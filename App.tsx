...
import { extractColumnData } from './services/aiService';
...
const MODELS = [
  ...
  { id: 'gemini-2.5-pro', name: 'Gemini 2.5 Pro', description: 'Balanced', icon: Cpu },
];
...
const App: React.FC = () => {
  ...
  const handleRunAnalysis = () => {
    console.log("handleRunAnalysis called", { documents: documents.length, columns: columns.length });
    if (documents.length === 0 || columns.length === 0) {
      console.log("Early return: no documents or columns");
      return;
    }
    console.log("Calling processExtraction");
  ...
  const processExtraction = async (docsToProcess: DocumentFile[], colsToProcess: Column[]) => {
    console.log("processExtraction started", { docs: docsToProcess.length, cols: colsToProcess.length });
    ...
    try {
      ...
      const tasks: { doc: DocumentFile; col: Column }[] = [];
        ...
        for (const doc of docsToProcess) {
          for (const col of colsToProcess) {
             // Only add task if result doesn't exist, is an error, or is empty
             const existingResult = results[doc.id]?.[col.id];
             const shouldProcess = !existingResult || 
                 existingResult.value?.startsWith('Error:') || 
                 existingResult.value === '' ||
                 (existingResult.status === 'needs_review' && existingResult.value === '');
             
             if (shouldProcess) {
                 tasks.push({ doc, col });
             } else {
                 console.log(`Skipping ${doc.name} - ${col.name}: already has result:`, existingResult.value?.substring(0, 50));
             }
          }
      }
      ...
      if (tasks.length === 0) {
        console.log("No tasks to process - all results already exist");
        console.log("To reprocess, clear the results first or they will be skipped");
        setIsProcessing(false);
        setColumns(prev => prev.map(c => colsToProcess.some(target => target.id === c.id) ? { ...c, status: 'completed' } : c));
        return;
      }
        console.log("start extract",tasks);
      ...
      const promises = tasks.map(async ({ doc, col }) => {
          ...
          try {
              console.log("About to call extractColumnData for", doc.name, col.name);
              const data = await extractColumnData(doc, col, selectedModel);
              console.log("extractColumnData returned:", data);
              if (controller.signal.aborted) {
                console.log("Aborted, not saving result");
                return;
              }

              console.log("Saving result for", doc.id, col.id, "Data:", data);
              // Validate data before saving
              if (!data || !data.value) {
                console.warn("Warning: Empty or invalid data returned for", doc.name, col.name, data);
              }
              setResults(prev => {
                  const newResults = {
                      ...prev,
                  ...
                  console.log("Updated results state for", doc.id, col.id, "New value:", newResults[doc.id]?.[col.id]?.value);
                  return newResults;
              });
              console.log("Result saved successfully");
          } catch (e: any) {
              console.error(`Failed to extract ${col.name} for ${doc.name}`, e);
              // Set error status for the column
              setColumns(prev => prev.map(c => 
                  c.id === col.id ? { ...c, status: 'error' } : c
              ));
              // Optionally show error in results
              setResults(prev => ({
                  ...prev,
                  [doc.id]: {
                      ...(prev[doc.id] || {}),
                      [col.id]: {
                          value: `Error: ${e?.message || 'Extraction failed'}`,
